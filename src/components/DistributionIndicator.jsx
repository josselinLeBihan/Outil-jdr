...
const DistributionIndicator = ({ title, standard, status }) => {
  return (
        ...
        {Object.entries(status).map(([key, statusData]) => {
          ...
          return (
            <div key={key} className={`p-2 rounded ${
              isOver ? 'bg-red-900/50 border border-red-600' : 
              isUnder ? 'bg-blue-900/50 border border-blue-600' : 
              'bg-green-900/50 border border-green-600'
            }`}>
              <div className="font-medium">Niveau {level}</div>
              <div className="text-xs">
                {statusData.current}/{statusData.target}
                {isOver && <span className="text-red-400 ml-1">(+{statusData.diff})</span>}
                {isUnder && <span className="text-blue-400 ml-1">({statusData.diff})</span>}
              </div>
            </div>
          );
        })}
